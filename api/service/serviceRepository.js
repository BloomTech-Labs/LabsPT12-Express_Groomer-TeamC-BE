...
class ServiceRepository extends Repository {
  ...
  async afterCreate(service, param) {
    ...
    try {
      ...
      const groomer = await Groomer.query()
        .where({ profile_id: param.context.profile.id })
        .first();
      if (!groomer)
        throw createHttpError(
          400,
          "Can't retrieve groomer information. Operation has been aborted."
        );
      ...
      const message =
        error.message ||
        `An error occurred while trying to link groomer to the created service. Created service id [${service[0].id}]`;
