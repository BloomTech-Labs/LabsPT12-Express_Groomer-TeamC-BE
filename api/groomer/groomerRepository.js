...
class GroomerRepository extends Repository {
    ...
    async getOne(id) {
        const result = await this.relatedOne("profiles.id", "profile_id", (obj) => obj.id === id)
        if (!result)
            throw new NotFound("Cloud find groomer with the specified id")
        return result
    }
    ...
    async beforeCreate(payload) {
        ...
        const groomer = await Profile.query().join("user_types", "user_types.id", "profiles.user_type")
            .where({"profiles.id": profile_id})
            .select("user_types.name as userType")
            .first()
        // is not groomer throw an error
        if (!groomer || (groomer && groomer.userType !== "groomer"))
            throw new UnauthorizedUser('Access denied. Only groomer are allowed to perform this operation.')   
