...
class Controller {
    ...
    async post(req, res) {
        try {
            ...
            if (body) {
                ...
                return res
                    .status(200)
                    .json({message: `${this.repository.model.constructor.name.toLowerCase()} has been created.`, result});
            }
    ...
    async put(req, res) {
        try {
            ...
            // if not, return the error with the status code 400 
            if (!body || (body && !('id' in body))) return res.status(400).json({message: 'Request does not contain body or "id" is undefined'})
            
            // call the update method from the repository
            const result = await this.repository.update(body.id, body, {context: req})
