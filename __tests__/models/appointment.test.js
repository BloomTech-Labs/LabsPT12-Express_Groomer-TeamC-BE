...
describe('testing model "Appointment"', () => {
    ...
    describe("persist data", () => {
        it("should successful create new appointment", async () => {
            ...
            const response = await GroomerService.create(serviceData)
    ...
    describe("delete data", () => {
        it("should delete row in the database table corresponding with the specified ID", async () => {
            ...
            const result = await Appointment.remove(appointment.id);
    ...
    describe("data type validation", () => {
        ...
        it("should failed if invalid data type", async () => {
            ...
            expect(result[4].message).toBe('does not conform to the "date-time" format')
    ...
    describe("check if data exist in the database", () => {
        it("should failed if incorrect client_id", async () => {
            ...
            expect(result[0].detail).toBe('[client_id] with value: [profile.id] does not exists in table: [profiles], column: [id]')
        })
        it("should failed if incorrect groomer_id", async () => {
            ...
            expect(result.length).toBe(1)
            expect(result[0].detail).toBe('[groomer_id] with value: [groomer.id] does not exists in table: [profiles], column: [id]')
        })
        it("should failed if incorrect service_id", async () => {
            ...
            expect(result.length).toBe(1)
            expect(result[0].detail).toBe('[service_id] with value: [groomerService.id] does not exists in table: [groomer_services], column: [id]')
        })
        it("should failed if incorrect animal_id", async () => {
            ...
            expect(result.length).toBe(1)
            expect(result[0].detail).toBe('[animal_id] with value: [animal.id] does not exists in table: [animals], column: [id]')
        })
    })
})